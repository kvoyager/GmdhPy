...
class NeuronTestCase(unittest.TestCase):
    ...
    def fit_classification_model(self, train_x, test_x, train_y, test_y, params):
    ...
    def test_plain_classification(self):
        ...
        train_x, test_x, train_y, test_y = train_test_split(
            data_x, label_y, test_size=0.33, random_state=3227
        )
        ...
        roc_auc = self.fit_classification_model(
            train_x, test_x, train_y, test_y, params
        )
        self.assertAlmostEqual(roc_auc, 0.75, places=tol_places)
        ...
        self.assertGreater(roc_auc, 0.6)
    ...
    def test_classification_with_validation(self):
        ...
        train_x, test_x, train_y, test_y = train_test_split(
            data_x, label_y, test_size=0.25, random_state=3227
        )
        ...
        pred_y = model.predict_proba(test_x)
        roc_auc = roc_auc_score(model.le.transform(test_y), pred_y)
        self.assertAlmostEqual(roc_auc, 0.74, places=tol_places)
