...
def _graph():
    return functools.partial(gv.Graph, format="svg")()


def _digraph():
    return functools.partial(gv.Digraph, format="svg")()
...
class PlotModel:
    ...
    def __init__(self, model, filename, plot_neuron_name=False, view=False):
        ...
        self.io_node_param = {
            "style": "filled",
            "color": self.io_pen_color,
            "fillcolor": self.io_node_color,
            "fontsize": "12",
        ...
        self.g.graph_attr.update(
            label="Self-organizing deep learning polynomial neural network\n ",
            labelloc="t",
            center="true",
            fontsize="18",
        )
    ...
    def _get_neuron_name(self, neuron):
        s = "layer {0}\nneuron {1}".format(
            neuron.layer_index, neuron.neuron_index, neuron.get_name()
        )
    ...
    def _add_connection(self, layers, neuron, u_index):
        input_is_original_feature, feature_index = self._get_feature_index(
            layers, neuron, u_index
        )
        if input_is_original_feature:
            return self._add_edge(
                self._get_feature_name(feature_index), self._get_neuron_name(neuron)
            )
        else:
            prev_layer = layers[neuron.layer_index - 1]
            parent_neuron = prev_layer[feature_index]
            return self._add_edge(
                self._get_neuron_name(parent_neuron), self._get_neuron_name(neuron)
            )
    ...
    def _add_edge(self, a, b):
        return self.g.edge(
            ...
            fillcolor=self.connection_fill_color,
            weight="1",
        )
