...
if __name__ == "__main__":
    ...
    if train_data_is_the_first_half:
        train_x = housing.data[:n]
        train_y = housing.target[:n]
        test_x = housing.data[n:]
        test_y = housing.target[n:]
    else:
        train_x = housing.data[n:]
        train_y = housing.target[n:]
        test_x = housing.data[:n]
    ...
    PlotModel(
        model, filename="california_housing_model", plot_neuron_name=True, view=True
    ).plot()
